...
int main(void) {
  ...
  Texture2D character =
      LoadTexture("D:\\Users\\dsfs\\Desktop\\Game\\George (1).png");
  Texture2D characterLittle =
      LoadTexture("D:\\Users\\dsfs\\Desktop\\Game\\Little_George.png");
  Texture2D characterJump =
      LoadTexture("D:\\Users\\dsfs\\Desktop\\Game\\George - Pulo.png");
  Texture2D characterLittleJump =
      LoadTexture("D:\\Users\\dsfs\\Desktop\\Game\\Little_George_Pulo.png");

  Texture2D enemyText =
      LoadTexture("D:\\Users\\dsfs\\Desktop\\Game\\goomba.png");

  Sound footstepSound = LoadSound(
      "D:\\Users\\dsfs\\Desktop\\Game\\Single-footstep-in-grass-A.mp3");
  Sound landingSound = LoadSound(
      "D:\\Users\\dsfs\\Desktop\\Game\\Single-footstep-in-grass-B.mp3");
  Sound soundtrack = LoadSound(
      "D:\\Users\\dsfs\\Desktop\\Game\\[8bit] Genshin Impact _ Faruzan Theme "
      "(Master of Ingenious Devices) [Chiptune Cover](MP3_160K).mp3");
  ...
  unsigned int numFrames =
      4; // Declarando o numero de frames da animacao 1 (correr)
  unsigned int numFrames2 =
      1; // Declarando o numero de frames da animacao 2 (pular)
  int frameWidth =
      character.width / numFrames; // Comprimento de um frame da animacao 1
  int frameLittleWidth = characterLittle.width / numFrames;
  int frame2Width =
      characterJump.width / numFrames2; // Comprimento de um frame da animacao 2
  ...
  Rectangle frameRec2 = {0.0f, 0.0f, (float)frame2Width,
                         (float)characterJump.height};
  Rectangle frameEnemy = {0.0f, 0.0f, (float)frameEnemyWidth,
                          (float)enemyText.height};

  Rectangle wall = {300, GetScreenHeight() / 2.0f - 50, 150,
                    165}; // parede teste

  Player player = {0};
  player.characterPos =
      (Vector2){groundYPos, screenHeight / 2.0f}; // Posicao inicial do player
  player.characterVelocity = (Vector2){0.0f, 0.0f};
  player.isJumping = false;

  Enemy enemy = {0};
  enemy.enemyPos = (Vector2){groundYPos - 170,
                             screenHeight / 1.6f}; // Posicao inicial do inimigo
  ...
  while (!WindowShouldClose()) {
    Rectangle hitbox = {hitboxPos.x, hitboxPos.y, hitboxWidth, hitboxHeight};
    enemy.enemyHeadHitbox.rect = (Rectangle){800, 450, 50, 10};
    enemy.enemyHitbox = (Rectangle){800, 450, 50, 60};

    //============================================INIMIGO======================================================
    if (enemyCollision1) {
      enemy.collision_flag++;
      enemy.collision_flag2++;
    }
    ...
    // printf("\t%d\n", IsCharacterOnGround(character, player.characterPos,
    // groundYPos, platforms, 4));
    ...
    if (IsCharacterOnGround(character, player.characterPos, groundYPos,
      ...
      else if(enemy.collision_flag2 != 0) {
        player.isJumping = false;
        player.characterVelocity.y = -3 * characterSpeed;
        
        enemy.collision_flag2 = 0;
      }   
    ...
    if (enemy.collision_flag == 0) {
        enemy.enemyHeadHitbox.rect.x = enemy.enemyPos.x;
        enemy.enemyHeadHitbox.rect.y = enemy.enemyPos.y;
        enemy.enemyHitbox.x = enemy.enemyPos.x;
        enemy.enemyHitbox.y = enemy.enemyPos.y + 10;
    } else {
        enemy.enemyHeadHitbox.rect.x = 0;
        enemy.enemyHeadHitbox.rect.y = 0;
        enemy.enemyHitbox.x = 0;
        enemy.enemyHitbox.y = 0;
    }

    wallCollision = CheckCollisionRecs(hitbox, wall); // Colosao com a parede teste
    enemyCollision1 = CheckCollisionRecs(enemy.enemyHeadHitbox.rect, (Rectangle){player.characterPos.x + 55, player.characterPos.y + 110, 40, 20}); // Colisao com a cabeca do inimigo
    enemyCollision2 = CheckCollisionRecs(enemy.enemyHitbox, hitbox); // Colisao com o inimigo (tira vida do George)
    ...
    DrawTexture(background, screenWidth / 2 - background.width / 2,
                screenHeight / 2 - background.height / 2, WHITE);
    ...
    if (wallCollision || enemyCollision2) {
      if (vida > 0) {
    ...
    if (isGameOver) {
      while (isGameOver) {
        ...
        DrawText("GAME OVER", 555, 350, 50, WHITE);
